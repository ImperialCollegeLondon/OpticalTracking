...
% Intraspecimen
config.postprocess.fill_missing_raw_data = @(x) deal(x, false(size(x))); % No interpolation

config.postprocess.fill_missing_quantisation = @(x) fillmissing(x, "pchip"); % Missing quanta are filled with this function. E.g., have flexion angles 40 and 42; it fills the 41 linearly.
config.postprocess.intraspecimen_smoothing = @(x) smoothdata(x, "gaussian", 10); % @(x) x => no smoothing

 % Interspecimen
config.postprocess.interpolation_algorithm = @(x, v, xq) interp1(x, v, xq, "pchip");
config.postprocess.interspecimen_smoothing = @(x) smoothdata(x, "gaussian", 10); 

config.postprocess.shift_flex = @(x) x; % No flexion shift
...
config.file_parser.right = {'rk', 'right', 'r'};
config.file_parser.left = {'lk', 'left', 'l'};

config.file_parser.digitisation = {'digit', 'calibr'}; % Case insensitive digitisation file names. If it contains any of these texts, treats it as a digitisation file
...
config.camera_labels.knee.polaris.tibia = 'T';
config.camera_labels.knee.polaris.femur = 'Y';
config.camera_labels.knee.polaris.patella = '';
config.camera_labels.knee.polaris.probe = 'Probe';
...
config.camera_labels.hip.polaris.femur = '';
