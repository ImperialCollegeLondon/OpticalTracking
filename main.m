...
label_probe_landmarks = "Probe";
label_tracker_tibia = "T";
label_tracker_femur = "Y";
...
landmarks = create_rigid_bodies(landmark_raw_data);
...
for i = 1:numel(data_raw)
    ...
    data(i).tibia = extract_from_label(data_sanitised(i).probes, label_tracker_tibia);
    data(i).femur = extract_from_label(data_sanitised(i).probes, label_tracker_femur);
