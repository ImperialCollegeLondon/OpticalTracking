...
folderPathDigitise=fullfile(pwd,'Amy Pilot Intact\Wooden Left Knee\Digitise');
folderPathData=fullfile(pwd,'Amy Pilot Intact\Wooden Left Knee\Data PF 16 - 22');
fileNameData=[folderPathData,'\Flexion Extension Normal.csv'];
...
% TIBIA AXIS POINTS
fileNameDigitise=[folderPathDigitise,'\Tibia medial.csv'];
rawData = xlsread(fileNameDigitise);
...
fileNameDigitise=[folderPathDigitise,'\Tibia lateral.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataT{2} = rawData(:,all(~isnan(rawData)));

fileNameDigitise=[folderPathDigitise,'\Tibia distal.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataT{3} = rawData(:,all(~isnan(rawData)));

% Insertion point of the patella tendon on the tibia
fileNameDigitise=[folderPathDigitise,'\Tibia PT Ins.csv'];
rawData = xlsread(fileNameDigitise);
...
% FEMUR AXIS POINTS
fileNameDigitise=[folderPathDigitise,'\Femur medial.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataF{1} = rawData(:,all(~isnan(rawData)));

fileNameDigitise=[folderPathDigitise,'\Femur lateral.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataF{2} = rawData(:,all(~isnan(rawData)));

fileNameDigitise=[folderPathDigitise,'\Femur proximal.csv'];
rawData = xlsread(fileNameDigitise);
...
fileNameDigitise=[folderPathDigitise,'\Patella medial.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataP{1} = rawData(:,all(~isnan(rawData)));

fileNameDigitise=[folderPathDigitise,'\Patella lateral.csv'];
rawData = xlsread(fileNameDigitise);
XYZdataP{2} = rawData(:,all(~isnan(rawData)));

fileNameDigitise=[folderPathDigitise,'\Patella PT Ins.csv'];
rawData = xlsread(fileNameDigitise);
...
ttrtPTc=gTtt0\grtPT0;


%% Load tracked points
 
rawData = xlsread(fileNameData);
