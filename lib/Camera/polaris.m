function [trackers, strays] = polaris(raw_headers, lines)
    ...
    has_strays = ~isempty(passive_strays_idx);
    if has_strays
    ...
    if has_strays
        for n = 1:size(data_stray, 1)
            tz = squeeze(data_stray(n, 1, :));
            ty = squeeze(data_stray(n, 2, :));
            tx = squeeze(data_stray(n, 3, :));
