...
function result = femoracetabular(relative_position, hip, femur, right)
    ...
    e3_=femur.k;%Tibial z axis in global reference frame, Grood and Suntay definition
    e2_=ucross(e3_,e1_);%Floating axis in global reference frame, Grood and Suntay definition

    flexion = asind(dot(-e2_,hip.k));
    beta = acosd(dot(hip.i,femur.k));
    if right
        external = asind(dot(-e2_,femur.i));
        abduction = 90-beta;
    else
        external = asind(dot(e2_,femur.i));
        abduction = -(90-beta);
    ...
    %     H_=femurOrigin-hipOrigin;
    H_ = femur.origin(1:3, :) - hip.origin(1:3, :);%translation vector of from femoral origin to femurl origin (in global coordinate frame)
    if right
        lateral = dot(H_,e1_);%projected onto the medial lateral axis e1
    else
        lateral = dot(H_,-e1_);
    ...
    result.flexion = flexion';
    result.abduction = abduction';
    result.adduction = -abduction';
    result.external = external';
    result.lateral = lateral';
    result.anterior = anterior';
    result.superior = distal';
    ...
    % fRtCheck{i,1}=[dot(hip.i,femur.i) dot(hip.i,femur.j) dot(hip.i,femur.k)
    %     dot(hip.j,femur.i) dot(hip.j,femur.j) dot(hip.j,femur.k)
    %     dot(hip.k,femur.i) dot(hip.k,femur.j) dot(hip.k,femur.k)];
    %
    % fRt{i,1}-fRtCheck{i,1};
